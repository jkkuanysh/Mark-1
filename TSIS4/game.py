"""
Core snake game logic for TSIS3.
"""
...
try:
    ...
    # Allows the game to still open if DB is not configured yet.
    save_result = None
    get_top_scores = None
...
def generate_food(snake, obstacles, foods, powerup):
    """Create weighted disappearing normal food."""
...
def generate_obstacles(level, snake):
    ...
    Keeps a safe zone around the snake head.
    """
    obstacles = set()
    if level < 3:
        return obstacles
...
def run_game(screen, clock, username, settings):
    ...
    while running:
        ...
        for ox, oy in obstacles:
            rect = pygame.Rect(ox * GRID_SIZE, TOP_BAR + oy * GRID_SIZE, GRID_SIZE, GRID_SIZE)
            pygame.draw.rect(screen, BLACK, rect)

        # Foods
        for food in foods:
            fx, fy = food["pos"]
            rect = pygame.Rect(fx * GRID_SIZE, TOP_BAR + fy * GRID_SIZE, GRID_SIZE, GRID_SIZE)
            pygame.draw.rect(screen, food["color"], rect)

        # Power-up
        if powerup:
            px, py = powerup["pos"]
            rect = pygame.Rect(px * GRID_SIZE, TOP_BAR + py * GRID_SIZE, GRID_SIZE, GRID_SIZE)
        ...
        for i, (sx, sy) in enumerate(snake):
            rect = pygame.Rect(sx * GRID_SIZE, TOP_BAR + sy * GRID_SIZE, GRID_SIZE, GRID_SIZE)
    ...
    return "game_over", game_over_data
...
def screen_main_menu(screen, clock, settings):
    ...
    while True:
        ...
        draw_text(screen, username or "Type here...", font, BLACK if username else GRAY, input_rect.x + 10, input_rect.y + 8)
...
def screen_leaderboard(screen, clock):
    ...
    while True:
        ...
        for i, row in enumerate(rows, start=1):
            ...
            date_str = played_at.strftime("%Y-%m-%d") if hasattr(played_at, "strftime") else str(played_at)[:10]
...
def screen_settings(screen, clock, settings):
    ...
    color_rects = [pygame.Rect(230 + i * 90, 320, 50, 50) for i in range(len(colors))]
